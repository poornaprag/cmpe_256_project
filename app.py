...
# Reading credentials file for Twitter
data_file = open("credentials.json", "r")
data = json.load(data_file)

# Getting all keys for Twitter
...
def get_sample_data():
    ...
    if result[0][0] == -1:
        ...
        positive = result[0][1]
    final_res = [
        {"y": int(positive), "label": "Postive"},
        {"y": int(negative), "label": "Negative"},
    ]
...
def data_user(username):
    ...
    return render_template(
        "data.html", title="Data We Use", tweets=get_tweets(username)
    )
...
def vis():
    ...
    print(sample_data)
    return render_template(
        "visual.html", title="Visualize", result=result, sample_data=sample_data
    )
